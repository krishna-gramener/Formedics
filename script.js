...
function saveToLocalStorage(analysis) {
  let saved = [];
  try {
    const stored = JSON.parse(localStorage.getItem('medicalTags') || '[]');
  ...
  localStorage.setItem('medicalTags', JSON.stringify(saved));
}
  ...
  try {
    ...
    const response = await fetch("https://llmfoundry.straive.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system", content: `You are assisting SMEs in evaluating medical content tagging results.
Analyze the feedback provided by the user and take one of the following actions:
Take ${previousData} as your primary data to work on.
1. **Update the Data:**
- If the feedback applies to a specific field or entry, update only that field or entry in the ${previousData}.
...
- If the SME explicitly asks for an explanation or detail, provide detailed reasoning or analysis based on the feedback without making changes to the ${previousData}.isValid in this case will be false.
...
correctedData should be in this format
                tags: [{text: string, confidence: number}],
                icd10: [{code: string, description: string, confidence: number}],
                mesh: [{term: string, code: string, confidence: number}],
                specialties: [{code: string, specialty: string, subspecialty: string}],
